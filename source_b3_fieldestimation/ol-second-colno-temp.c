...
struct param pars={ULONG_MAX,1,1,1,50,.SIGMA=1.0,.AR_SIZE=0,.maxr=0.0};
...
void show_options(char *progname)
{
  ...
  fprintf(stderr,"\t-T temperature quotient [default %.2lf]\n",pars.SIGMA);
...
int main(int argc,char **argv)
{
  ...
  while (count <FLENGTH) {
    ...
    make_cast_no(cast,new,pars,drift,gamma,diffusion);
