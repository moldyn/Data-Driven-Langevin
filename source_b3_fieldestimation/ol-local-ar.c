...
char dimset=0,nofields=0,fullembed=0;
...
void show_options(char *progname)
{
  ...
  fprintf(stderr,"\t-E do full embed [default no]\n");
...
int main(int argc,char **argv)
{
  ...
  double *new,**coeff,*diffusion,*loav;
  struct sfound sf;
  FILE *fout,*ffield;
  ...
  check_alloc(loav=(double*)malloc(sizeof(double)*(pars.DIM*(pars.EMB+1))));
  ...
  if (pars.maxr > 0.0)
    ...
    for (i=0;i<pars.DIM*pars.EMB;i++)
      fprintf(ffield,"av_%d ",i+1);
    for (i=0;i<pars.DIM;i++)
      fprintf(ffield,"av_f%d ",i+1);
    for (i=0;i<pars.DIM;i++)
      for (j=0;j<pars.DIM*pars.EMB;j++)
	fprintf(ffield,"c_%d_%d ",i+1,j+1);
    for (i=0;i<pars.DIM;i++)
  ...
  while (count <FLENGTH) {
    ...
    get_ar_coeff(series,pars,sf,loav,coeff,diffusion);
    make_ar_cast(cast,new,pars,loav,coeff,diffusion);
	...
	for (j=0;j<pars.EMB;j++)
      ...
      for (i=0;i<pars.DIM*pars.EMB;i++) {
	j=i/pars.EMB;
	fprintf(ffield,"%e ",loav[i]*max+sermin[j]);
      }
      for (i=0;i<pars.DIM;i++)
	fprintf(ffield,"%e ",loav[pars.DIM*pars.EMB+i]*max+sermin[i]);
      for (i=0;i<pars.DIM;i++)
	for (j=0;j<pars.DIM*pars.EMB;j++)
