...
int main(int argc,char **argv)
{
  ...
  pars.minminn=(unsigned long)((3.0*pars.DIM+3.0)/2.0+0.5);
  ...
  if (!noiseonly) {
    for (i=0;i<pars.DIM;i++)
      fprintf(fout,"f%d ",i+1);
    for (i=0;i<pars.DIM;i++)
      for (j=0;j<pars.DIM;j++)
	fprintf(fout,"g_%d_%d ",i+1,j+1);
    for (i=0;i<pars.DIM;i++)
      for (j=0;j<=i;j++)
	fprintf(fout,"K_%d_%d ",i+1,j+1);
    if (!reference) {
      for (i=0;i<pars.DIM;i++)
  ...
  while (n < FLENGTH) {
    ...
    /* calculate fields */
    get_fields_no(series,pars,sf,drift,gamma,diffusion,cast);

    /* calculate noise */
    if (!reference)
      make_test_no(series,refn,new,pars,drift,gamma,diffusion);
