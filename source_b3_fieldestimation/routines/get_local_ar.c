		  ...
		  double *av,double **coef,double *dif)
{
  ...
  for (i=0;i<p.DIM;i++) {
	...
	j1=j/p.EMB;
	j2=j%p.EMB;
	h +=  coef[i][j]*(series[j1][which-j2]-av[j]);
      }
      dif[i] += sqr(series[i][which+1]-av[p.DIM*p.EMB+i]-h);
    }
...
void make_ar_cast(double **x,double *new,struct param p,double *av,
		  double **coeff,double *dif)
{
  ...
  for (d=0;d<p.DIM;d++) {
    new[d] *= dif[d];
    new[d] += av[p.EMB*p.DIM+d];
    for (i=0;i<p.DIM*p.EMB;i++) {
      i1=i/p.EMB;
      i2=i%p.EMB;
      new[d] += coeff[d][i]*(x[i1][p.hdim-i2]-av[i]);
    }
  }
}
