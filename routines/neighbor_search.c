...
void put_in_boxes(int which,double **x,struct param p,unsigned int *fut)
{
  long n,i,j,k,N;
  unsigned int hdim;
  long ***box,*list;
  ...
  for (n=hdim;n<p.LENGTH;n++) {
    if (fut[n]) {
      i=(long)(x[0][n]*scale)%N;
      j=(long)(x[nsseconddim][n]*scale)%N;
      k=(long)(x[nsthirddim][n]*scale)%N;
      list[n]=box[i][j][k];
      box[i][j][k]=n;
    }
  }
...
void sort(unsigned long nfound,struct param p,struct sfound sf)
{
  double dswap,*distance,distsigma,diffr;
  unsigned long i,j,iswap,*f;
  ...
  if (p.maxr > 0.0) {
    ...
    if (sf.count[0] == 0)
      ...
      distsigma=sf.aveps[0]/(double)sf.count[0];
  }
#endif
...
#if defined(WEIGHTS)
  if (distance[minminn] > distsigma)
    diffr=distance[minminn];
  else
    diffr=distsigma;
  ...
  for (i=0;i<p.MINN;i++) {
    if (distance[i] <= diffr) {
      ...
      sf.weight[i]=exp(-(distance[i]-diffr)/(diffr*WFACT));
    }
  }
#else
  for (i=0;i<p.MINN;i++)
  ...
  sf.count[0]++;
  sf.aveps[0] += distance[p.MINN];
}

unsigned int find_neighbors(double **x,double **y,unsigned int box,
			    struct param p,struct sfound sf,double eps)
{
  long i,j,k,i1,i2,j1,j2,k1,k2,ld,le,x2,y2;
  long element;
  unsigned long nfound=0,*f;
  double hmax,dx,eps2,*distance;
  long ***hfbox,*hflist;
  ...
  if (y[0][hdim]>=1.0) 
    ...
    if (y[0][hdim]<0.0) i=0;
    else i=((long)(y[0][hdim]*N));
  }
  if (y[nsseconddim][hdim]>=1.0) 
    j=N-1;
  else {
    if (y[nsseconddim][hdim]<0.0) j=0;
    else j=((long)(y[nsseconddim][hdim]*N));
  }
  if (y[nsthirddim][hdim]>=1.0) 
    k=N-1;
  else {
    if (y[nsthirddim][hdim]<0.0) k=0;
    else k=((long)(y[nsthirddim][hdim]*N));
  }
