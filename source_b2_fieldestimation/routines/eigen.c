...
void my_eigen(double **mat,unsigned long n,double *eig)
{
  double *trans,*off,*matarr;
  int ierr,i,j,nm=(int)n;

  check_alloc(trans=(double*)malloc(sizeof(double)*nm*nm));
  check_alloc(matarr=(double*)malloc(sizeof(double)*nm*nm));
  check_alloc(off=(double*)malloc(sizeof(double)*nm));

  for (i=0;i<nm;i++)
    for (j=0;j<nm;j++)
      matarr[i+j*nm]=mat[i][j];

  tred2(&nm,&nm,matarr,eig,off,trans);
...
void get_diffusion(double **mat,struct param p)
{
  unsigned long i,j,k;
  double **ret,*eig;

  ret=(double**)malloc(sizeof(double*)*p.DIM);
  eig=(double*)malloc(sizeof(double)*p.DIM);
  for (i=0;i<p.DIM;i++)
    ret[i]=(double*)malloc(sizeof(double)*p.DIM);
  my_eigen(mat,p.DIM,eig);
  for (i=0;i<p.DIM;i++)
    eig[i]=sqrt(eig[i]);
  for (i=0;i<p.DIM;i++)
    for (j=0;j<p.DIM;j++) {
