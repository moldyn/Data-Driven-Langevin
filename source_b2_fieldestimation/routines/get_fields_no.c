...
double *av_fut,*av_past,**mat_pastpast,**mat_futpast,**mat_futfut;
		   ...
		   double *drift,double **gamma,double **dif,double **x)
{
  ...
  double *sd,*sd1,sum,num,**imat,hf,hp,sweights,*weight,hw;
...
#if defined(WEIGHTS)
	hw=weight[i];
	hp += sd[fim1]*sd1[fim1]*hw;
	hf += sd[fi1]*sd1[fi1]*hw;
      }
...
/* Cholesky decomposition to obtain K */
  ...
  for (d=0;d<p.DIM;d++) {
    drift[d]=av_fut[d]-x[d][hdim];
    for (k=0;k<p.DIM;k++)
      drift[d] += gamma[d][k]*(x[k][hdim]-av_past[k]);
  }
}
