...
void mat_init(struct param p)
{
  ...
  check_alloc(avv=(double*)malloc(sizeof(double)*p.DIM));
  isinitgetfields=1;
}
		...
		double *drift,double **dif)
{
  ...
  double *sd,*sd1,sum,num,h,hv,sweights,*weight;
  unsigned long *f;
  ...
  for (d=0;d<p.DIM;d++) {
	...
	fi1=fi+1;
#if defined(WEIGHTS)
	sum += (sd[fi1]-sd[fi])*(sd1[fi1]-sd1[fi])*weight[i];
      }
      mat[d][d1]=(sum/sweights-avv[d]*avv[d1]);
      mat[d1][d]=mat[d][d1];
#else
	sum += (sd[fi1]-sd[fi])*(sd1[fi1]-sd1[fi]);
      }
      mat[d][d1]=num/(num-1.0)*(sum/num-avv[d]*avv[d1]);
      mat[d1][d]=mat[d][d1];
#endif
    }
  }
